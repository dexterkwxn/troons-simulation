 ...
 * veque.hpp
 *
 * Efficient generic C++ container combining useful features of std::vector and
 * std::deque
 *
...
// Resizing behavior resembling std::vector.  Also ideal for queue-like
// push_back/pop_front behavior.
...
template <typename T, typename ResizeTraits = fast_resize_traits,
...
public:
  ...
  template <typename InputIt, typename ItCat = typename std::iterator_traits<
  ...
  veque(const veque &other)
      : veque(_allocate_uninitialized_tag{}, other.size(),
  ...
  veque(veque &&other) noexcept { _swap_with_allocator(std::move(other)); }
  ...
  template <typename OtherResizeTraits>
  veque(veque<T, OtherResizeTraits, Allocator> &&other,
    ...
    if constexpr (!alloc_traits::is_always_equal::value) {
      if (alloc != other._allocator()) {
        ...
        auto replacement =
            veque(_allocate_uninitialized_tag{}, other.size(), alloc);
        _nothrow_move_construct_range(other.begin(), other.end(),
                                      replacement.begin());
  ...
  veque &operator=(const veque &other) { return _copy_assignment(other); }
  ...
  void assign(std::initializer_list<T> lst) { _assign(lst.begin(), lst.end()); }
  ...
  reference at(size_type idx) {
    if (idx >= size()) {
      throw std::out_of_range("veque<T,ResizeTraits,Alloc>::at(" +
  ...
  // Iterators
  const_iterator cbegin() const noexcept { return _storage_begin() + _offset; }

  iterator begin() noexcept { return _storage_begin() + _offset; }
  ...
  iterator end() noexcept { return _storage_begin() + _offset + size(); }
  ...
  void reserve(size_type front, size_type back) {
    if (front > capacity_front() || back > capacity_back()) {
      ...
      if (new_full_capacity > max_size()) {
        throw std::length_error("veque<T,ResizeTraits,Alloc>::reserve(" +
                                std::to_string(front) + ", " +
                                std::to_string(back) + ") exceeds max_size()");
      }
      _reallocate(new_full_capacity, allocated_before_begin);
    }
  }

  void reserve_front(size_type count) { reserve(count, 0); }

  void reserve_back(size_type count) { reserve(0, count); }

  void reserve(size_type count) { reserve(count, count); }
  ...
  // To achieve interface parity with std::vector, capacity() returns
  // capacity_back();
  ...
  void clear() noexcept {
    ...
    if constexpr (std::ratio_greater_v<_unused_realloc, std::ratio<0>>) {
      using unused_front_ratio =
          std::ratio_divide<_front_realloc, _unused_realloc>;
      _offset =
          capacity_full() * unused_front_ratio::num / unused_front_ratio::den;
    }
  }
  ...
  template <typename... Args> reference emplace_front(Args &&...args) {
    ...
    alloc_traits::construct(_allocator(), begin() - 1,
  ...
  void resize_front(size_type count) { _resize_front(count); }
  ...
  void resize_back(size_type count) { _resize_back(count); }

  void resize_back(size_type count, const T &value) {
    _resize_back(count, value);
  }

  // To achieve interface parity with std::vector, resize() performs
  // resize_back();
  void resize(size_type count) { _resize_back(count); }

  void resize(size_type count, const T &value) { _resize_back(count, value); }
  ...
  void swap(veque<T, OtherResizeTraits, Allocator> &other) noexcept(
    ...
    if constexpr (alloc_traits::propagate_on_container_swap::value) {
      ...
      if (_allocator() == other._allocator()) {
        ...
        // std::vector would declare this UB.  Allocate compatible storage and
        // make it work.
        auto new_this =
            veque(_allocate_uninitialized_tag{}, other.size(), _allocator());
        _nothrow_move_construct_range(other.begin(), other.end(),
                                      new_this.begin());

        auto new_other =
            veque(_allocate_uninitialized_tag{}, size(), other._allocator());
...
private:
  ...
  static constexpr auto _resize_from_closest_side =
      ResizeTraits::resize_from_closest_side;
  ...
  static_assert(std::ratio_greater_equal_v<_front_realloc, std::ratio<0>>,
                "Reserving negative space is not well-defined");
  ...
  // Confirmation that allocator_traits will only directly call placement
  // new(ptr)T()
  ...
  struct Data : Allocator {
    ...
    Data(size_type size, const Allocator &alloc)
        : Allocator{alloc},
          _storage{size ? std::allocator_traits<Allocator>::allocate(
                              allocator(), size)
                        : nullptr},
          _allocated{size} {}
    ...
    ~Data() {
      if (_storage) {
        std::allocator_traits<Allocator>::deallocate(allocator(), _storage,
                                                     _allocated);
      }
    }
  ...
  veque(InputIt b, InputIt e, __attribute__((unused)) const Allocator &alloc,
  ...
  veque(_allocate_uninitialized_tag, size_type size, size_type allocated,
        size_type offset, const Allocator &alloc)
  ...
  // Create an uninitialized empty veque, with storage for expected reallocated
  // size
  veque(_reallocate_uninitialized_tag, size_type size, const Allocator &alloc)
      : veque(_allocate_uninitialized_tag{}, size, _calc_reallocation(size),
              _calc_offset(size), alloc) {}
  ...
  template <typename OtherResizeTraits>
  veque &
  _move_assignment(veque<T, OtherResizeTraits, Allocator> &&other) noexcept(
      noexcept(alloc_traits::propagate_on_container_move_assignment::value ||
               alloc_traits::is_always_equal::value)) {
    if constexpr (!alloc_traits::is_always_equal::value) {
      if (_allocator() != other._allocator()) {
        if constexpr (alloc_traits::propagate_on_container_move_assignment::
          ...
          if (other.size() > capacity_full()) {
            ...
            _reassign_existing_storage(std::move_iterator(other.begin()),
  ...
  template <typename... Args>
  void _value_construct_range(const_iterator b, const_iterator e,
                              const Args &...args) {
    static_assert(sizeof...(args) <= 1,
                  "This is for default- or copy-constructing");

    if constexpr (std::is_trivially_copy_constructible_v<T> &&
                  _calls_default_constructor_directly) {
  ...
  void _copy_construct_range(It b, It e, const_iterator dest) {
    ...
    if constexpr (std::is_trivially_copy_constructible_v<T> &&
                  _calls_copy_constructor_directly) {
  ...
  template <typename It> void _assign(It b, It e) {
    static_assert(std::is_convertible_v<
                  typename std::iterator_traits<It>::iterator_category,
                  std::forward_iterator_tag>);
  ...
  template <typename It> void _assign(It b, It e, std::forward_iterator_tag) {
    _assign(b, e);
  }

  template <typename It> void _assign(It b, It e, std::input_iterator_tag) {
  ...
  template <typename It> iterator _insert(const_iterator it, It b, It e) {
    static_assert(std::is_convertible_v<
                  typename std::iterator_traits<It>::iterator_category,
                  std::forward_iterator_tag>);
  ...
  template <typename OtherResizeTraits>
  void _swap_with_allocator(
      veque<T, OtherResizeTraits, Allocator> &&other) noexcept {
    // Swap everything
  ...
  void _reallocate(size_type allocated, size_type offset) {
    auto replacement = veque(_allocate_uninitialized_tag{}, size(), allocated,
                             offset, _allocator());
    _nothrow_move_construct_range(begin(), end(), replacement.begin());
    _swap_without_allocator(std::move(replacement));
  }

  // Insert empty space, choosing the most efficient way to shift existing
  // elements
  iterator _insert_storage(const_iterator it, size_type count) {
    ...
    auto replacement =
        veque(_reallocate_uninitialized_tag{}, required_size, _allocator());
    auto index = std::distance(cbegin(), it);
    auto insertion_point = begin() + index;

    _nothrow_move_construct_range(begin(), insertion_point,
                                  replacement.begin());
    _nothrow_move_construct_range(insertion_point, end(),
                                  replacement.begin() + index + count);
  ...
  void _shift_front(const_iterator b, const_iterator e, size_type count) {
    ...
    if (element_count > 0) {
      ...
      if constexpr (std::is_trivially_copyable_v<T> &&
        ...
        auto src = start;
        auto dest_construct_end =
            std::min(begin(), _mutable_iterator(e) - count);
  ...
  // Moves a range towards the back.  Veque will grow, if needed.  Vacated
  // elements are destructed. Moves a valid subrange in the back direction.
  ...
  template <typename It> void _reassign_existing_storage(It b, It e) {
    static_assert(std::is_convertible_v<
                  typename std::iterator_traits<It>::iterator_category,
                  std::forward_iterator_tag>);
    ...
    if (size() == 0) {
      ...
      // Existing veque is the same size.  Avoid any construction by
      // copy-assigning everything
      std::copy(b, e, begin());
      return;
    } else if (size_delta < 0) {
      // New size is smaller.  Copy-assign everything, placing results as close
  ...
  void _reassign_existing_storage(size_type count, const T &value) {
    ...
    // The "ideal" begin would put the new data in the center of storage
    if constexpr (std::ratio_greater_v<_unused_realloc, std::ratio<0>>) {
      using ideal_begin_ratio =
          std::ratio_divide<_front_realloc, _unused_realloc>;
      ideal_begin += (capacity_full() - count) * ideal_begin_ratio::num /
                     ideal_begin_ratio::den;
    }
  ...
  void _nothrow_move_construct(iterator dest, iterator src) {
    if constexpr (std::is_trivially_copy_constructible_v<T> &&
      ...
      *dest = *src;
      ...
      alloc_traits::construct(_allocator(), dest,
                              _nothrow_construct_move(*src));
    }
  }
  ...
  void _move_end(difference_type count) noexcept { _size += count; }
  ...
  const_iterator _storage_begin() const noexcept { return _data._storage; }

  iterator _storage_begin() noexcept { return _data._storage; }
};

template <typename T, typename LResizeTraits, typename LAlloc,
          typename RResizeTraits, typename RAlloc>
inline bool operator==(const veque<T, LResizeTraits, LAlloc> &lhs,
                       const veque<T, RResizeTraits, RAlloc> &rhs) {
...
inline void
swap(veque<T, ResizeTraits, Alloc> &lhs,
     veque<T, ResizeTraits, Alloc> &rhs) noexcept(noexcept(lhs.swap(rhs))) {
  lhs.swap(rhs);
}
...
veque(InputIt, InputIt, Alloc = Alloc())
    -> veque<typename std::iterator_traits<InputIt>::value_type,
             fast_resize_traits, Alloc>;
